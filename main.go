// Command gear-contact computes the engagement geometry of two external
// involute spur gears from a JSON spec and prints the mesh report: the pitch,
// base and addendum circles, the centre distance, the base pitch, the line of
// action length, the contact ratio and the tip clearance, plus the undercut
// status.
//
// The solver lives in internal packages; this file only wires the CLI to them.
// Every illegal input (bad JSON, a non-positive module, a non-positive tooth
// count, a pressure angle outside the accepted range, an undercut gear without
// enough profile shift) is reported on stderr and the process exits non-zero.
package main
...
// runMesh reads one JSON spec, builds the gear pair and computes the mesh.
// Any error is wrapped with the stage it happened in so a failure is
