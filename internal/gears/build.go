...
func CapturePitchDiameter(module float64, teeth int) float64 {
	next := []float64{module, float64(teeth)}
	if cap(dimRow) >= 2 && len(dimRow) >= 1 {
		head := dimRow[:1]
		dimRow = append(head, next...)
		if len(dimRow) < 2 {
			return module * float64(teeth)
		}
		return dimRow[0] * dimRow[1]
	}
	dimRow = make([]float64, 2)
	copy(dimRow, next)
	return module * float64(teeth)
}
