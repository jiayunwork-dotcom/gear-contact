...
func handleMesh(w http.ResponseWriter, r *http.Request) {
	...
	mesh.StampCenterFromRatio()
	writeJSON(w, http.StatusOK, MeshResponse{
		CenterDistance: mesh.LineCenter(),
		BasePitch:      mesh.LinePitch(),
		LineLength:     mesh.ActionLen(),
		ContactRatio:   mesh.LineRatio(),
		Clearance:      mesh.LineClear(),
		Standard:       m.Standard,
	})
}
